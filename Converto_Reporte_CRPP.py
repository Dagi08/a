...
def Corto_Plazo_Mun(directorio_in, directorio_out, keywords):

    def filter_data(data, keywords):
        pattern = '|'.join(keywords)
        return data[data['CLIENTE'].str.contains(pattern, case=False, na=False)]
    ...
    df= resultado_final.drop([resultado_final.columns[8], resultado_final.columns[9], resultado_final.columns[10] ,resultado_final.columns[11], resultado_final.columns[12], resultado_final.columns[13]], axis=1)  
            ...
            if r_idx == 1:  # Si es la primera fila (encabezados)
                ...
                if 'F_VENCIMIENTO' in df.columns and c_idx == df.columns.get_loc('F_VENCIMIENTO') + 1:
                    ...
                    cell.number_format = 'DD/MM/YYYY'
                            ...
                            df.columns.get_loc('SALDO_script2') + 1,
...
def Corto_plazo_No_Mun(directorio_in, directorio_out, keywords):
    def filter_data(data, keywords):
        pattern = '|'.join(keywords)
        return data[data['CLIENTE'].str.contains(pattern, case=False, na=False)]
...
def Largo_Plazo_Mun(directorio_in, directorio_out, keywords):
...
def Largo_Plazo_No_Mun(directorio_in, directorio_out, keywords):
...
def CRPP_Hasta_4301a(directorio_in, directorio_out):
    # Definir el ancho fijo de las columnas crpp4601
...
def CRPP_Mayor_4301a(directorio_in, directorio_out):
    # Definir el ancho fijo de las columnas crpp4601
    column_widths = [20, 31, 18, 9, 14]
    column_names = ["N°CUENTA", "PRESTATARIO", "SALDO", "TASA_EA", "SALDOxTASA"]
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.title("SECCION SOPORTE SERVICIOS FINANCIEROS (SSSF)")
...
class FileFrame(BaseFrame):
    def __init__(self, master):
        ...
        self.File_Entry_up.configure(state='readonly')
    ...
    def UpFile(self):
        ...
        if file_path:
            self.File_Entry_up.configure(state='normal')
...
class HomeFrame(BaseFrame):
    def __init__(self, master):
        ...
        self.Search_Entry.bind('<KeyRelease>', self.validate_and_uppercase)

        self.Search_Button= customtkinter.CTkButton(master=self.Search_Frame, text="Buscar", font=customtkinter.CTkFont(size=15,weight="bold"), border_width=1, 
                                                    border_color="#BF0615", fg_color="#FFFFFF", text_color="#BF0615", hover_color="#F2CDD0",command=self.get_keywords)
        ...
        self.cPlazo_button_ejecutar.configure(state="disabled")

    def validate_and_uppercase(self, event):
        current_text = self.Search_Entry.get()
        uppercase_text = current_text.upper()
        if current_text != uppercase_text:
            self.Search_Entry.delete(0, 'end')
            self.Search_Entry.insert(0, uppercase_text)

    def get_keywords(self):
        keywords = self.Search_Entry.get().upper()
        self.master.keywords = [word.strip() for word in keywords.split(',') if word.strip()]
        if self.master.keywords:
            self.cPlazo_button_ejecutar.configure(state="normal")  # Habilitar el botón después de buscar
        CTkMessagebox(title="Palabras clave", message=f"Palabras clave establecidas: {', '.join(self.master.keywords)}", icon="info", option_1="OK", bg_color="#FFFFFF", 
        ...
        if hasattr(self.master, 'directorio_in') and hasattr(self.master, 'directorio_out'):
            try:  
                if self.radio_var.get() == 0:
                    Corto_Plazo_Mun(self.master.directorio_in, self.master.directorio_out, self.master.keywords)
                    CTkMessagebox(message="Datos extraidos exitosamente.",
                        ...
                        Corto_plazo_No_Mun(self.master.directorio_in, self.master.directorio_out, self.master.keywords)
                        CTkMessagebox(message="Datos extraidos exitosamente.",
                        icon="check", option_1="Ok", bg_color="#FFFFFF", fg_color="#FFFFFF", button_color="#4D4D4D", button_hover_color="#BF0615", button_text_color="#FFFFFF")
            except Exception as e:
                ...
                # Reiniciar el estado del radio button, bloque el button y borra el entry después de la ejecución
...
class SecondFrame(BaseFrame):
    ...
    def Ejecutar_LargoPlazo(self):
        if hasattr(self.master, 'directorio_in') and hasattr(self.master, 'directorio_out'):
            try:  
                if self.radio_var.get() == 0:
                    Largo_Plazo_Mun(self.master.directorio_in, self.master.directorio_out, self.master.keywords)
                    CTkMessagebox(message="Datos extraidos exitosamente.",
                                  icon="check", option_1="Ok", bg_color="#FFFFFF", fg_color="#FFFFFF", button_color="#4D4D4D", button_hover_color="#BF0615", button_text_color="#FFFFFF")
                elif self.radio_var.get() == 1:
                        Largo_Plazo_No_Mun(self.master.directorio_in, self.master.directorio_out, self.master.keywords)
                        CTkMessagebox(message="Datos extraidos exitosamente.",
                        icon="check", option_1="Ok", bg_color="#FFFFFF", fg_color="#FFFFFF", button_color="#4D4D4D", button_hover_color="#BF0615", button_text_color="#FFFFFF")
            except Exception as e:
                CTkMessagebox(message=f"Ocurrió un error: {str(e)}",icon="cancel", option_1="Thanks", bg_color="#FFFFFF", 
...
class ThirdFrame(BaseFrame):
    ...
    def Ejecutar_CRPP4301a(self):
        if hasattr(self.master, 'directorio_in') and hasattr(self.master, 'directorio_out'):
            try:
                CRPP_Hasta_4301a(self.master.directorio_in, self.master.directorio_out)
                CRPP_Mayor_4301a(self.master.directorio_in, self.master.directorio_out)
                CTkMessagebox(message="Datos extraidos exitosamente.",
                    icon="check", option_1="Ok", bg_color="#FFFFFF", fg_color="#FFFFFF", button_color="#4D4D4D", button_hover_color="#BF0615", button_text_color="#FFFFFF")
            except Exception as e:
                CTkMessagebox(message=f"Ocurrió un error: {str(e)}",icon="cancel", option_1="Thanks", bg_color="#FFFFFF", 
